...
export default function FeatureFull({ feature, index, number }: { feature: Feature; index: number; number: number }) {
  ...
  return (
    <article id={`f-${feature.key}`} className="scroll-mt-36 grid items-center gap-8 lg:grid-cols-2 lg:gap-14">
      <Reveal className={flip ? "lg:order-2" : ""}>
        <div className={flip ? "v-slide-r" : "v-slide-l"}>
          <div className="flex items-center gap-3">
            ...
            <span className="v-fade font-mono text-xs font-semibold tracking-[0.2em] text-slate-300" style={stagger(1)}>
          ...
          <div className="v-rise mt-6 rounded-xl border border-slate-200 bg-slate-50/70 p-4" style={stagger(9)}>
            ...
            <div className="mt-2 flex flex-wrap gap-1.5">
              {feature.screens.map((s) => (
                <span key={s} className="rounded-md bg-white px-2 py-1 text-[11px] font-medium text-slate-600 ring-1 ring-slate-200 transition-colors hover:text-slate-900 hover:ring-brand-200">
                  {s}
                </span>
              ))}
            </div>
      ...
      <Reveal delay={120} className={flip ? "lg:order-1" : ""}>
        {/* The picture arrives from the opposite side, then breathes. */}
        <div className={`${flip ? "v-slide-l" : "v-slide-r"} group/pic`}>
          <div className="v-float transition-transform duration-500 group-hover/pic:-translate-y-1 group-hover/pic:scale-[1.015]">{Picture && <Picture />}</div>
        </div>
      </Reveal>
    </article>
  );
