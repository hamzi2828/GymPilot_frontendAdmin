...
export default function Hero() {
  return (
    <section id="product" className="relative overflow-hidden bg-slate-950 pb-20 pt-32 text-white sm:pt-36 lg:pb-28 lg:pt-40">
      ...
      <Glow className="left-1/2 top-0 h-[720px] w-[1200px] -translate-x-1/2 -translate-y-1/3" color="rgba(79,70,229,0.4)" drift="a" />
      <Glow className="right-0 top-1/3 h-[560px] w-[560px] translate-x-1/3" color="rgba(217,70,239,0.22)" drift="b" />

      <div className="relative mx-auto grid max-w-7xl items-center gap-14 px-5 sm:px-8 lg:grid-cols-[1.05fr_1fr] lg:gap-10">
        <div>
          <span className="a-rise inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 py-1 pl-1.5 pr-3 text-xs font-medium text-slate-300" style={stagger(0)}>
          ...
          <h1 className="mt-6 font-display text-[2.75rem] font-extrabold leading-[1.02] tracking-[-0.03em] sm:text-6xl lg:text-[4.25rem]">
            <span className="a-rise block" style={stagger(1)}>
              {HERO.title[0]}
            </span>
            <span className="a-rise block" style={stagger(2)}>
              {HERO.title[1]}
            </span>
            <span className="a-rise text-gradient block" style={stagger(3)}>
              {HERO.title[2]}
            </span>
          </h1>

          <p className="a-rise mt-6 max-w-xl text-lg leading-relaxed text-slate-300" style={stagger(4)}>
            {HERO.lead}
          </p>

          <div className="a-rise mt-8 flex flex-col gap-3 sm:flex-row sm:items-center" style={stagger(5)}>
            <a href={HERO.primary.href} className="btn-shine btn-shine-dark group inline-flex h-12 items-center justify-center gap-2 rounded-full bg-white px-6 text-sm font-semibold text-slate-900 shadow-lg shadow-white/10 transition-transform hover:-translate-y-0.5">
          ...
          <ul className="mt-8 flex flex-wrap gap-x-6 gap-y-2 text-sm text-slate-400">
            {HERO.trust.map((t, i) => (
              <li key={t} className="a-rise flex items-center gap-2" style={stagger(6 + i)}>
          ...
          {SITE.demoGymUrl && (
            <p className="a-rise mt-6 text-xs text-slate-500" style={stagger(9)}>
        ...
        <div className="a-rise relative lg:pl-6" style={stagger(3)}>
          <ProductMock />
        </div>
      </div>
    </section>
